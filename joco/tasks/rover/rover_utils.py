...
F_MAX = 5.0  # max possible objective value
...
class RoverDomain:
    ...
    def __init__(
        ...
        goal,
        traj,
        n_samples,  # prev versions used 1k
        force_start=True,
        force_goal=False,
        rnd_stream=None,
    ):
    ...
    def __call__(self, params):
        traj_points = self.trajectory(params)
        cost = self.traj_to_cost(traj_points)
        return -1 * cost
    ...
    def estimate_cost(self):
        ...
        points = self.traj.get_points(
            np.linspace(0, 1.0, self.n_samples, endpoint=True)
        )
        ...
        l1 = np.linalg.norm(points[1:] - points[:-1], axis=1)
        total_cost = np.sum(l1 * avg_cost)
    ...
    def traj_to_cost(self, traj_points):
        # compute cost at each point
        costs = self.cost_fn(traj_points)

        # estimate (trapezoidal) the integral of the cost along traj
        avg_cost = 0.5 * (costs[:-1] + costs[1:])
        l1 = np.linalg.norm(traj_points[1:] - traj_points[:-1], axis=1)
        total_cost = np.sum(l1 * avg_cost)

        assert self.force_start
        if not self.force_goal:
            total_cost += 100 * np.linalg.norm(traj_points[-1] - self.goal, 1)
        return total_cost

    def trajectory(self, params):
        self.set_params(params)
        return self.traj.get_points(np.linspace(0, 1.0, self.n_samples, endpoint=True))

    def trajectory_length(self, params):
    ...
    def distance_from_goal(self, params):
        self.set_params(params)
        points = self.traj.get_points(
            np.linspace(0, 1.0, self.n_samples, endpoint=True)
        )
...
def create_cost_large():
    # original obstacle setup from TuRBO paper
    c = np.array(
        [
    ...
    return cost_fn, start, goal, l1, obstacle_delta
